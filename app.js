...
// ملاحظة: لجعل البيانات تظهر على الموبايل، نستخدم هنا LocalStorage محلياً
// ولكن الحل الفعلي هو وضع الرابط السحابي للسيارات
...
function saveNewCar() {
    ...
    alert("تمت الإضافة بنجاح! ملاحظة: لكي تظهر على جهاز آخر، يجب أن يكون التطبيق مرتبطاً بسيرفر.");
}
    ...
    list.innerHTML = myBookings.map(b => `
        <div class="glass rounded-[30px] p-6 border-white/5 flex justify-between items-center mb-4">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 ${b.status === 'approved' ? 'premium-gradient' : 'bg-slate-800'} rounded-full flex items-center justify-center text-white">
                    <i class="fas ${b.status === 'approved' ? 'fa-check' : 'fa-clock'}"></i>
                </div>
                <div><h4 class="font-bold text-white text-base">${b.carModel}</h4><p class="text-[10px] text-slate-500 mt-1">${b.date}</p></div>
            </div>
            <div class="text-[10px] px-4 py-2 rounded-full border ${b.status === 'approved' ? 'text-green-400 border-green-400/20' : 'text-amber-500 border-amber-500/20'} font-bold">
                ${b.status === 'approved' ? 'مؤكد' : 'قيد الانتظار'}
            </div>
        </div>
    `).reverse().join('');
}
...
function confirmBooking() {
    ...
    userBookings.push({ id: Date.now(), userPhone: currentUser.phone, carId: car.id, carModel: car.model, date: new Date().toLocaleDateString('ar-SA'), status: 'pending' });
    localStorage.setItem('rim_bookings', JSON.stringify(userBookings));
...
function handleSearch(q) {
    ...
    filtered.forEach(car => {
        ...
        card.innerHTML = `<div class="relative h-64"><img src="${car.image}" class="w-full h-full object-cover"></div><div class="p-8"><h3 class="text-xl font-bold text-white mb-4">${car.model}</h3><button onclick="bookCar(${car.id})" class="w-full py-5 rounded-[22px] premium-gradient text-white font-bold">حجز الآن</button></div>`;
        grid.appendChild(card);
    });
}
